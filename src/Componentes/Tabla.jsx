...
export default function CustomPaginationActionsTable(props) {
  ...
  function createData(habilidad, descripcion) {
    const object = { nro: contador, habilidad, descripcion };
  ...
  React.useEffect(() => {
    for (let i = 0; i < array.length; i++) {
      const nuevoElemento = createData(...array[i]);
      filas.push(nuevoElemento);
      setFilas(filas);
            ...
            <TableHead>
              <TableRow>
                {headers.map((header, index) => (
                  <TableCell key={index} align="rigth">
                    {header}
                  </TableCell>
                ))}
              </TableRow>
